...
const characters = [
  ...
  { name: "Ince Zangwill" }, { name: "Katarina Pelle" }, { name: "Klein Moretti" }, { name: "Lenevus" }, 
  ...
  { name: "Sasir" }, { name: "Sauron-Einhord-Medici" }, { name: "Siatas" }, { name: "Snowman" }, 
...
function dailyDoneKey() { return "lotmdlequote_daily_done_" + todayKey(); }
...
function infSaveKey() { return "lotmdlequote_infsave_v1"; }
...
function clearPractiseState() { localStorage.removeItem(infSaveKey()); }
...
let mode = localStorage.getItem("lotmdlequotemode") || "daily";
...
function streakKeyQuoteDaily() {
  ...
  return `lotmdlequote_dailystreak_${uid}`;
}

function streakKeyQuoteInf() {
  const uid = auth?.currentUser?.uid || "guest";
  return `lotmdlequote_infstreak_${uid}`;
}
...
function setMode(newMode) {
  ...
  mode = newMode;
  localStorage.setItem("lotmdlequotemode", mode);
  syncModeUI();
...
function hideEndScreen() {
  ...
  if (window.lotmdleNextDailyTimerQuote) clearInterval(window.lotmdleNextDailyTimerQuote);
}
...
function showEndScreen(won) {
  ...
  if (mode === "daily") {
    ...
    clearPractiseState(); // POPRAWKA: było "clearInfiniteState()"
    if (playAgainBtn) {
...
function openLb() {
  ...
  if(typeof loadLeaderboardLoggedIn === "function") loadLeaderboardLoggedIn("dailyquote");
}
...
// Init
(function init() {
  const KEY = "lotmdlequote_practise_hard_reset_v2";
